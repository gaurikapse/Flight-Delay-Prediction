...
Departures data can be accessed [here](https://www.dubaipulse.gov.ae/data/dubai-airports-flight-info/da_flight_information_departures-open) and weather data can be accessed [here](https://www.ncei.noaa.gov/access/search/data-search/global-summary-of-the-day?stations=41194099999&startDate=2020-01-01T00:00:00&endDate=2023-01-01T23:59:59).

For a description of the weather data, click [here](https://www.ncei.noaa.gov/data/global-summary-of-the-day/doc/readme.txt).
"""

# Load departures data from csv
...
departures = pd.read_csv("data/Flight_Information_Departures.csv")

# Calculate delay from timestamps
departures['delay'] = pd.to_timedelta(pd.to_datetime(departures['actualOffblockTime']) - pd.to_datetime(departures['scheduledOffblockTime']), unit = 'm')

# Replace the destinationName column with the viaName column if viaName is not null
# This is because we only consider direct flights
departures['destinationName'] = np.where(departures['viaName'].notnull(), departures['viaName'], departures['destinationName'])

# Add a date column (extract dates from schedule) for merge with weather data
departures['DATE'] = pd.to_datetime(departures['publicScheduledDateTime']).dt.date
...
# Now merge the departures and weather data
data = departures.merge(weather, on='DATE', how='left')
...
* 'arrivalOrDeparture' - all flights are departures
...
* 'viaName' - imputed into destinationName
* 'destination_iata', 'destination_icao' - same as originName
* 'airlineNameA' - airline name in Arabic
* 'destinationNameA' - origin name in Arabic
...
* 'scheduledOffblockTime', 'actualOffblockTime' - these were used to calculate the delay
* 'estimatedOffblockTime' - this is an estimate that DXB airport officials make
...
* 'actualTakeOffTime' - this occurs after actualOffBlockTime so the delay has been accounted for
* 'checkinAllocationFrom', 'checkinAllocationTo' - checkin usually closes well ahead of scheduled departure, so the checkin counter assigned shouldn't affect departure
* 'publicGateNumber' - linked to aircraftParkingPosition
"""

data.drop(columns = ['aodbUniqueField', 'trafficTypeCode', 'flightStatus', 'flightStatusCode', 
                     'aircraftRegistration', 'arrivalOrDeparture', 'lastChanged', 'airlineCode_iata', 
                     'airlineCode_icao', 'via_iata', 'via_icao', 'viaName', 'destination_iata', 'destination_icao',
                     'airlineNameA', 'destinationNameA', 'viaNameA', 'flightStatusTextA', 'destination_icao', 
                     'aircraft_icao', 'scheduledOffblockTime', 'actualOffblockTime', 'estimatedOffblockTime', 
                     'delayZScore', 'STATION', 'LATITUDE', 'LONGITUDE', 'ELEVATION', 'NAME', 'SNDP', 'MXSPD',
                     'SLP', 'TEMP_ATTRIBUTES', 'DEWP_ATTRIBUTES', 'SLP_ATTRIBUTES', 'STP_ATTRIBUTES', 'VISIB_ATTRIBUTES', 
                     'WDSP_ATTRIBUTES', 'MAX_ATTRIBUTES', 'MIN_ATTRIBUTES', 'PRCP_ATTRIBUTES', 'MAX', 
                     'MIN', 'actualTakeOffTime', 'checkinAllocationFrom', 'checkinAllocationTo', 'publicGateNumber'], 
          inplace = True)
...
# There are many airlines and not all of them have operations at DXB on the same scale
...
# Now drop jointFlightNumber, publicScheduledDateTime, FRSHTT
data.drop(columns = ['jointFlightNumber', 'publicScheduledDateTime', 'FRSHTT', 'Snow', 'Tornado'], inplace = True)
...
model.fit(X_train, y_train, cat_features = ['Flight Number', 'Aircraft Type', 'Parking Position', 'Airline', 'Destination City', 'Traffic Type', 'Terminal'], logging_level = 'Silent')
...
#     candidates = output.loc[(output['Prediction'] == 0) & (output['Destination City'] == test_point['Destination City']) & (output.Date.isin(date_range)), :]
#     candidates.loc[:,'Date'] = candidates.Date.dt.strftime("%Y-%m-%d")
#     similarFlights = candidates[:5][['Flight Number', 'Airline', 'Destination City', 'Date']].to_dict('record')
...
path = '/content/gdrive/MyDrive/CSE6242/results/departure_prediction_outputs.csv'
